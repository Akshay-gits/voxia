...
const MirrorSpeak = () => {
  ...
  // Improved speech-to-text configuration with better mobile support
  const { error, interimResult, isRecording, results, startSpeechToText, stopSpeechToText } = useSpeechToText({
    ...
    crossBrowser: true,
    timeout: 30000, // Increased timeout for better mobile performance
    objectMode: true,
  });
  ...
  const startRecording = async () => {
    try {
      ...
      // Specify mimeType conditionally based on browser support
      let options = {};
      ...
      mediaRecorder.onstop = () => setRecordedVideo(URL.createObjectURL(new Blob(chunksRef.current, { type: options.mimeType || 'video/webm' })));
      mediaRecorder.start(1000);
      
      // Start speech recognition after media recorder is set up
      setTimeout(() => {
        startSpeechToText();
      ...
      try {
        ...
        setTimeout(() => {
          startSpeechToText();
          console.log("Speech to text started (fallback)");
        }, 500);
      } catch (fallbackErr) {
  ...
  // Mobile-optimized recording view
  const renderMobileRecordingView = () => (
    <div className="relative h-screen w-full bg-black">
      ...
      {/* Topic display */}
      <div className="absolute top-4 right-4 bg-indigo-600 text-white px-3 py-1 rounded-full text-sm max-w-full truncate">
        Topic: {currentTopic}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 text-white p-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <button onClick={() => setShowTranscript(!showTranscript)} className="text-indigo-300 text-xs">
          ...
          <div className="max-h-32 overflow-y-auto">
            {transcript.length > 0 ? (
              transcript.slice(-3).map((line, i) => (
                <p key={i} className="text-gray-200 border-l-2 border-indigo-400 pl-2 py-1 text-sm">{line}</p>
              ))
            ) : !interimResult ? (
              <p className="text-gray-400 italic text-sm">Start speaking to see your transcript...</p>
            ) : null}
            
            {interimResult && (
              <p className="text-gray-400 italic border-l-2 border-gray-500 pl-2 py-1 text-sm">{interimResult}</p>
            )}
          </div>
        )}
        ...
        <div className="mt-4 flex justify-center">
          <button 
            ...
            className="bg-red-600 text-white px-8 py-2 rounded-full font-medium hover:bg-red-700 transition-colors flex items-center justify-center"
          >
  ...
  const renderStage = () => {
    const stages = {
      ...
      recording: isMobile ? renderMobileRecordingView() : (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white text-gray-800 font-sans">
      {/* Mobile-specific header for recording stage */}
      {stage === 'recording' && isMobile ? (
        <div className="fixed top-0 left-0 right-0 bg-black bg-opacity-70 text-white p-4 z-10">
      ...
      {/* Mobile footer for non-recording stages */}
      {stage !== 'recording' && (
